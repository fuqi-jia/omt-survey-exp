...
主 run 结束后须补做,完成前本 run 的数据视为**中间态**:

1. 删除并重跑 `B_jobshop_10_6 / scip-bigm`(§8.2 的并发触及)与
   `B_jobshop_10_7 / highs-bigm`(该格求解期间本机同时编译了一次论文 PDF)。
   两者均为单核负载、机器 16 线程,实际影响预计不可测量,但该两格进入论文表格,
   重跑成本仅各 ≤600 s,故不以"影响很小"代替重测。
2. 跑满 10 个种子后,以 `scripts/make_jobshop_table.py` 重新生成表体并回填论文。

本次提交时 `results.jsonl` 为 **8 个完整种子**(seed 0--7);论文 `67eee9b`
所引用的是其中 **7 个种子**(seed 0--6)的数据,可由该脚本加 `--seeds 7` 复现。
